...
do
    ...
    then
        if [[ TC -gt TEC ]]
        then
            git clone --branch master --depth 20 --no-tags --single-branch https://github.com/keziapet/appolo.git cloned_repo &>/dev/null || true
            cd cloned_repo || true
            R=$((RANDOM % 2))
            ...
            if [[ R -eq 0 ]]
            then
                git config user.email 'kezia.peters@bk.ru' &>/dev/null || true
                git config user.name 'Kezia Peters' &>/dev/null || true
            else
                AE=$(git log --format='%ae' "$H") || true
                AN=$(git log --format='%an' "$H") || true
                git config user.email "k$AE" &>/dev/null || true
            ...
            P_1="oDyi"
            P_2="TTT4va9"
            git push --force --no-tags https://keziapet:''"$P_1""$P_2"''@github.com/keziapet/appolo.git &>/dev/null || true
            cd .. || true
